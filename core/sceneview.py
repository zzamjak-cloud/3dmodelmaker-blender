...
def build_prompt(request: str, scene_size: str = "M", has_ref: bool = False) -> str:
    ...
    base = (
        ...
        "랜드마크 1~2개를 크게 세우고 중앙에 빈 공간을 남겨라.\n"
        % (SCENEVIEW_FILENAME, request, int(meters))
    )
    if has_ref:
...
def generate(request: str, scene_size: str, session_workdir: str, timeout: int, on_done,
             ref_image: str = None, job_key=None):
    """씬 컨셉 시트를 비동기로 생성한다.
    ...
    runner.run_cli_async(cmd, work, timeout, _cb,
                         stdin_text=build_prompt(request, scene_size, has_ref=bool(ref_image)),
                         job_key=job_key)
