...
- **캐릭터**: 인간형·동물형·크리처형 게임 캐릭터. **원화(참조 이미지)** 를 넣으면 그 캐릭터를 3x2 **턴어라운드 시트**(정면 | 뒷면 | 좌측면 / 우측면 | 상면 | 3/4뷰, 리깅용 중립 자세)로 먼저 전개한다. 이후 경로는 두 가지다:
  - **이미지→3D 셰이프 (기본, 로컬 Hunyuan3D 서버 필요)**: 시트의 정면·뒷면·좌·우측면을 잘라 로컬 Hunyuan3D-2mv 서버에 보내 하이폴리 셰이프를 받고, 선택한 방식으로 게임용 메시를 만든다. **베이스 메시 템플릿**은 편집 가능한 얼굴·관절 루프를 유지하며 몸체 표면에 제한적으로 적합하고, **QuadriFlow**는 복셀 리메시 후 균일 쿼드를 만든다. **데시메이트**는 삼각형으로 줄인다. 키(기본 1.8m)·발바닥 z=0 정규화 후 텍스처 베이크로 이어진다. 기존 저장된 환경설정의 리토폴로지 방식은 유지된다.
...
분리 모드에는 `preserve_parts`를 지원하는 저장소의 최신 `scripts/hunyuan3d/lp3d_h3d_server.py`가 필요하다.
기존 서버가 이 기능을 지원하지 않으면 상태에 업데이트 안내를 표시한다. 서버를 갱신·재시작한 뒤
`/status` 응답에서 `capabilities.preserve_parts: true`를 확인한다.
기존 Hunyuan 설치 폴더에 서버 파일을 복사할 필요 없이, 기존 서버를 종료한 뒤 다음처럼
저장소 서버를 실행할 수도 있다(Windows 예시).

```powershell
& 'D:\Tools\Hunyuan3D-2\.venv\Scripts\python.exe' `
  '.\scripts\hunyuan3d\start_server.py' --hunyuan-root 'D:\Tools\Hunyuan3D-2' --port 8081
```
...
`scripts/hunyuan3d/lp3d_h3d_server.py`는 원본 `api_server.py`를 대체하는 서버다 — 멀티뷰 입력(`front`/`back`/`left`/`right` base64)을 받고, 통합 생성에서는 파편 제거(FloaterRemover)를 적용한다. 분리 생성의 `preserve_parts` 요청에서는 작은 장비도 남긴다. 생성 중에도 `/status`가 응답한다. 요청 형식은 원본과 호환이라 Blender MCP의 Hunyuan3D `LOCAL_API` 모드도 같은 서버를 쓸 수 있다. 텍스처 파이프라인(C++/CUDA 확장)은 빌드하지 않는다 — 텍스처는 애드온의 6면도 베이크가 담당한다.

환경설정 → **캐릭터**: 셰이프 생성 켜기/끄기, 서버 주소(기본 `http://127.0.0.1:8081`), 리토폴로지 목표 면수(기본 12,000 쿼드 ≈ 24,000 tris), 리토폴로지 방식, 캐릭터 기본 키. 통합 생성에서 서버가 응답하지 않으면 패널 로그에 남기고 코드 모델링으로 폴백한다. 템플릿 방식은 원형의 면수를 최대 두 단계 subdivision하므로 목표 면수와 정확히 같지는 않다.
