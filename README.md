...
- **캐릭터**: 인간형·동물형·크리처형 게임 캐릭터. **원화(참조 이미지)** 를 넣으면 그 캐릭터를 3x2 **턴어라운드 시트**(정면 | 뒷면 | 좌측면 / 우측면 | 상면 | 3/4뷰, 리깅용 중립 자세)로 먼저 전개하고, 시트를 기준으로 모델링한 뒤 모델링 타입(컬러 스와치 / 개별 매핑)에 따라 매핑까지 진행한다. 원화가 없으면 프롬프트만으로 시트를 만든다.
