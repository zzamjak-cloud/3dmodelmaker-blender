...
**매핑 시점**(잡 옵션, 개별 매핑일 때): 기본 "모델링 후 수동"은 리토폴로지가 끝나면 상태 **모델링 완료**로 멈춘다. 메시를 원하는 만큼 수정한 뒤 잡 패널의 **[매핑 시작]**을 누르면 그 시점의 메시를 그대로 언랩·6면도 채색·베이크한다(부품은 순차, 각 부품 시트를 색 참조로). 매핑에 필요한 정보(요청문·턴어라운드·스타일·유형)는 컬렉션 커스텀 프로퍼티에 남아 Blender를 다시 열어도 유효하며, 매핑이 실패하면 메시는 그대로 두고 다시 누를 수 있다. "자동"은 기존처럼 리토폴로지 직후 매핑까지 이어간다.
