...
- `size_class`는 에셋 1개의 트라이 상한을 정한다: **L 6000 / M 2500 / S 800**.
- `zone`은 반드시 `zones`에 있는 `name` 중 하나여야 한다.
- `landmark`는 씬의 시선을 잡는 주 구조물에만 `true` (1~2개).
- 씬 전체 예상 트라이 = Σ(`count` × size_class 상한) + 지형 여유. 주어진 예산을 넘기지 마라.

# 씬 구성 규칙 (캐주얼 배경의 핵심)

1. **랜드마크 1~2개**: 씬에서 가장 큰 질량 하나를 정하고 사실 대비 **130~160%**로 과장하라. 랜드마크가 없으면 공간이 평평하게 읽힌다.
2. **반복 프랍은 3~4종으로 제한**: 종류를 늘리지 말고 같은 프랍을 **회전·스케일 지터**로 변주해 밀도를 채워라. 종류가 많을수록 드로우콜과 제작 시간만 늘고 통일감은 깨진다.
3. **씬 팔레트 3~5색 + 악센트 1색**: 지면·구조물·자연물의 주조색을 먼저 정하고, 채도 높은 악센트는 랜드마크와 동선 표시에만 쓴다.
4. **빈 공간 40% 이상**: 모바일 화면에서 읽히려면 비어 있는 바닥이 필요하다. 중앙 광장·연병장·마당처럼 **의도적으로 빈 구역**을 하나 두어라.
...
2. **키트에 없는 프랍을 새로 모델링하지 마라**: 새로 만들어도 되는 것은 지형, 담장/벽(`wall_run`), 길(`path_strip`), 그리고 계단·받침·표지판 같은 **단순 box 구조물**뿐이다.
