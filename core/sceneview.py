...
def _body(request: str, scene_size: str = "M", has_ref: bool = False,
    ...
    if interior:
        ...
        base = (
            "대상: %s (건물 내부)\n"
            "규모: 대상에 맞는 실제 크기의 실내 공간(참고 한 변 %dm — 거실·침실이면 4~6m처럼 대상에 맞게). "
            "가구를 실제 비율로 그리고 방이 가구에 비해 휑하게 넓지 않게 하라. 치수 숫자·화살표는 그리지 마라.\n"
        ...
        base = (
            "대상: %s\n"
            "규모: 대상에 맞는 실제 크기의 부지(참고 긴 변 %dm — 대상에 맞게 더 작거나 커도 된다). 건물·차량·텐트를 실제 비율로 그리고 "
            "치수 숫자·화살표는 그리지 마라. **부지를 정사각형으로 그리지 마라** — 강·해안·"
            "능선·숲 경계를 따라 비정형 윤곽(길쭉하거나 굽은 형태)으로 잡고, 길은 굽은 유기적 "
            "곡선으로, 건물은 길과 광장 주변에 뭉치게 그려라. 격자 배열과 직각 경계는 "
            "계획도시를 요청받았을 때만 쓴다.\n"
