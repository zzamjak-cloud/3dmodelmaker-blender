...
- **캐릭터**: 인간형·동물형·크리처형 게임 캐릭터. **원화(참조 이미지)** 를 넣으면 그 캐릭터를 3x2 **턴어라운드 시트**(정면 | 뒷면 | 좌측면 / 우측면 | 상면 | 3/4뷰, 리깅용 중립 자세)로 먼저 전개한다. 이후 경로는 두 가지다:
  ...
  - **캐릭터 유형** 드롭다운(자동 / 인간형 / 동물형 / 크리처형)이 비율·골격 규칙을 정한다 — 자동이면 요청문·원화에서 판단한다. 인간형은 두신 비율(스타일이 정함)과 T-포즈(양팔 수평 — 옆면 매핑에 팔이 겹치지 않게), 동물형은 실제 골격 비율과 관절 방향(앞다리 팔꿈치 뒤·뒷다리 무릎 앞), 크리처형은 동물 부위 조합이되 하나의 골격 논리.
  - 시스템 프롬프트(`prompts/system_character.md`)는 리깅을 전제로 한다: T-포즈/네 발 중립 자세, 정면 -Y, 발바닥 z=0, 중심선 X=0, 반쪽 모델링 + `lp.mirror_x`, **관절 단위 파트 분할**(머리/목/가슴/골반/상완/하완/손/대퇴/하퇴/발), 파트 사이 0.05m 틈, 머리·얼굴에 트라이 25~35%, 장비는 별개 오브젝트로 join 허용. 시트와 요청문이 다르면 시트가 이긴다.
...
- **몸체 / 의상 / 무기 분리**: 부위별 시트와 셰이프를 순서대로 생성한다. 원본에 없는 장비는 빈 시트로
  확인되면 생략한다. 각 부위는 독립 메시로 유지하고 옷 아래 몸체 면도 남긴다.
  개별 매핑은 부위마다 별도 가이드·참조 시트·텍스처로 처리한다. 부위 생성 실패는 실패 상태로 표시한다.
- 부품 위치는 공통 시트 실루엣의 축척·위치를 이용한 추정이다. 이미지 모델이 자세·여백을 바꾸거나
  원화에 가려진 부분이 많으면 손으로 부착 위치를 보정해야 한다. 시트·셰이프·텍스처 요청이 늘어나므로
  통합 생성보다 오래 걸린다.

분리 모드의 부위는 **몸체 / 의상·악세사리 / 무기·방어구** 3종이다(순서대로 생성). 악세사리를 따로 생성하지 않는 이유: 벨트·고리처럼 몸을 감싸는 소품만 시트에 남기면 TRELLIS.2 가 고리 안쪽을 덩어리로 채워 슬랩이 된다(입력 뷰가 깨끗해도 재현). **의상·악세사리는 '옷 입은 전신'으로 생성한 뒤 몸체를 뺀다** — 사람 없는 옷만 그린 시트로는 모델이 속이 빈 옷을 만들지 못한다(실측: 좌우 반쪽으로 분리, 반바지 소실, 세로 2.4배 눌림). 의상 시트는 무기·방어구만 제거한 전신이고, 리토폴로지·배치 뒤 같은 축척의 몸체 메시를 기준으로 **몸체 표면에서 키의 1.5%(≈2.7cm) 안에 있거나 몸체 내부인 면을 지우고** 전체 면수 0.3% 미만 파편(피부·머리카락 잔재)을 버린다(`retopo.carve_body`, BVH 근접 판정). 불리언 차감은 팽창 사본의 자기교차(손가락·머리카락)로 EXACT 솔버가 부분 결과를 내서 쓰지 않는다. 결과 의상은 쿼드가 유지된 열린 셸이며, 몸에 그 거리보다 밀착한 옷은 함께 사라지므로 모델링 완료 단계에서 손본다. 지울 면이 없거나 전부 지워지거나, 정리 뒤에도 조각이 8개를 넘거나 45% 미만만 남으면(두 생성물이 어긋나 옷까지 지운 것) 차감을 되돌려 옷 입은 전신 셸을 그대로 둔다(로그 '몸체 차감 취소'). **실측(2026-09-18)**: 몸체와 옷 입은 전신은 따로 생성되어 팔·머리카락이 수 cm 어긋나므로 2.7cm 임계에서는 셔츠가 사라지고 피부·머리카락 조각 9개가 남았고, 5.4cm·9cm에서는 옷의 11%·4%만 남았다 — 자동 차감은 대개 취소되고, 그래서 자동 삭제는 환경설정 `의상에서 몸체 근접 면 자동 삭제 (실험)`(기본 꺼짐)으로 내렸고, 기본 산출물은 **몸체(맨몸) + 옷 입은 전신** 두 파트다. 옷 속 몸체 면은 모델링 완료 단계에서 사용자가 지운다(부품 모드는 은면 정리를 하지 않는다). 각 오브젝트에는 `lp3d_character_part`(부위)와 `lp3d_part_sheet`(그 부위의 시트 경로)가 커스텀 프로퍼티로 남는다. 몸체 외 부품은 시트를 만들기 전에 **원화를 비전 모델(codex CLI)에 보여 유무를 YES/NO로 묻고**, NO 면 시트·셰이프를 건너뛴다(CLI 가 없거나 답을 못 읽으면 시트 생성으로 진행). 생성 모델은 "원본에 없으면 빈 시트" 지시를 자주 어기고 검·방패 같은 부품을 지어내며(실측: 몸 실루엣 안에 크게 그려 실루엣 게이트를 통과한 사례), 색 비교 게이트는 나무 방패색이 조끼색과 겹쳐 판별력이 없었다. 2차 방어선으로, 부품 시트의 정면 실루엣 중 **원본 캐릭터 실루엣 밖에 있는 비율이 50% 이상이면 없는 부품으로 판정해 생략**한다(셰이프 비용을 쓰지 않음, 로그에 비율 표시). 리토폴로지 목표 면수는 몸체만 환경설정 값을 그대로 받고, 나머지 부품은 **몸체 대비 정면 실루엣 면적비**(^0.75, 하한 12%)로 배분한다 — 벨트 하나가 몸체와 같은 12,000면을 받지 않게. 부품 시트의 칸 크기가 몸체 시트와 다르면(격자선 검출 차이) 4%까지는 무시, 15%까지는 로그만 남기고 정규화 좌표로 진행, 그 이상은 그 부품만 생략한다(잡 전체를 실패시키지 않음). 시트 분할은 등분이 아니라 **격자선을 찾아** 자르고(격자선은 충분히 어둡고 **얇아야** 한다 — 격자선 없는 시트에서 3/4뷰 몸통 열이 격자선으로 잡혀 의상이 생략된 실측 뒤 이웃 대비 조건 추가) 라벨·격자 잔재를 흰색으로 덧칠한다(생성 모델이 격자를 균등하게 그리지 않아 옆 칸 라벨이 섞이면 셰이프에 바닥판이 생기고 부품 배치가 무너진다 — 실측).

**매핑 시점**(잡 옵션, 개별 매핑일 때): 기본 "모델링 후 수동"은 리토폴로지·조합이 끝나면 상태 **모델링 완료**로 멈춘다. 메시를 원하는 만큼 수정한 뒤 잡 패널의 **[매핑 시작]**을 누르면 그 시점의 메시를 그대로 언랩·6면도 채색·베이크한다(부품은 순차, 각 부품 시트를 색 참조로). 매핑에 필요한 정보(요청문·턴어라운드·스타일·유형)는 컬렉션 커스텀 프로퍼티에 남아 Blender를 다시 열어도 유효하며, 매핑이 실패하면 메시는 그대로 두고 다시 누를 수 있다. "자동"은 기존처럼 리토폴로지 직후 매핑까지 이어간다.

분리 모드에는 `preserve_parts`를 지원하는 저장소의 최신 `scripts/trellis3d/` 서버가 필요하다.
서버가 이 기능을 지원하지 않으면 상태에 업데이트 안내를 표시한다. 서버를 재배포한 뒤
`/status` 응답에서 `capabilities.preserve_parts: true`를 확인한다.
