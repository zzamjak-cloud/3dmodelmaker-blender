...
분리 모드의 부위는 **몸체 / 의상·악세사리 / 무기·방어구** 3종이며, **각 부품은 독립 잡처럼** 만들어진다: 원본 턴어라운드(없으면 원화)를 디자인 참조로 첨부해 부품만 캔버스에 크게 그린 자체 3x2 턴어라운드를 생성하고, 그 6면으로 셰이프를 만들어 **전체 면수 예산**으로 리토폴로지한 뒤 몸체 원점·오른옽으로 나란히 배치한다(`lp3d_character_part`, `lp3d_part_sheet` 프로퍼티). 조합(위치·축척 미세 조정)과 옷 속 몸체 면 정리는 사용자가 모델링 완료 단계에서 한다. 부품 키는 몸체는 환경설정 키, 나머지는 시트 생성 전 **원화를 비전 모델(codex CLI)에 보여 유무와 키 대비 비율을 묻는 판독**(`YES 0.65` / `NO`)에서 얻고, 판독이 없으면 기본 비율(의상 0.8·무기 0.6)을 쓴다. NO 면 시트·셰이프를 건너뛴다(빈 시트도 생략). 이전(v0.19~v0.21.0)의 결합 구조 — 원본과 같은 카메라·축척 강제, 실루엣 기반 자동 배치·면수 배분·환각 게이트, 옷 입은 전신에서 몸체 차감 — 는 부품마다 다시 그려진 시트와 따로 재구성된 셰이프가 수 cm 씩 어긋나 조합 결과가 통합 생성보다 나빴기에(실측 2026-09-18) 제거했다. 속이 빈 옷은 셰이프 모델의 약점(반쪽 분리·눌림이 재현됨)이라 의상 품질은 통합 생성보다 낮을 수 있다 — 캐릭터 한 덩어리가 필요하면 통합 생성을 쓴다.
