...
- **캐릭터**: 인간형·동물형·크리처형 게임 캐릭터. **원화(참조 이미지)** 를 넣으면 그 캐릭터를 3x2 **턴어라운드 시트**(정면 | 뒷면 | 좌측면 / 우측면 | 상면 | 3/4뷰, 리깅용 중립 자세)로 먼저 전개한다. 이후 경로는 두 가지다:
  - **이미지→3D 셰이프 (기본, 셰이프 서버 필요)**: 시트의 정면·뒷면·좌·우측면을 잘라 셰이프 서버(TRELLIS.2 — 각자 자기 Modal 계정에 배포하거나 로컬 GPU에서 실행, 아래 "캐릭터 셰이프 서버" 참고)에 보내 하이폴리 셰이프를 받고, 선택한 방식으로 게임용 메시를 만든다. **베이스 메시 템플릿**은 편집 가능한 얼굴·관절 루프를 유지하며 몸체 표면에 제한적으로 적합하고, **QuadriFlow**는 복셀 리메시 후 균일 쿼드를 만든다. **데시메이트**는 삼각형으로 줄인다. 키(기본 1.8m)·발바닥 z=0 정규화 후 텍스처 베이크로 이어진다. 기존 저장된 환경설정의 리토폴로지 방식은 유지된다.
  - **코드 모델링 (통합 생성 폴백)**: 통합 생성에서 서버가 없거나 셰이프 생성이 실패하면 시트를 보고 Astra가 리깅 자세로 코드 모델링하고, 실행 결과를 시트와 같은 6시점으로 렌더해 **시트와 대조하는 턴**을 돌려 빠진 요소·비율·떨어진 파트를 고친다(환경설정 `캐릭터 6면도 대조 횟수`, 기본 1). 분리 모드는 부위 보존을 지원하는 서버가 필요하며, 서버가 없으면 안내 후 종료한다.
  - 원화가 없으면 프롬프트만으로 시트를 만든다.
  - **캐릭터 유형** 드롭다운(자동 / 인간형 / 동물형 / 크리처형)이 비율·골격 규칙을 정한다 — 자동이면 요청문·원화에서 판단한다. 인간형은 두신 비율(스타일이 정함)과 A-포즈(양팔을 45도 아래로 — 베이스 메시 템플릿과 같은 자세), 동물형은 실제 골격 비율과 관절 방향(앞다리 팔꿈치 뒤·뒷다리 무릎 앞), 크리처형은 동물 부위 조합이되 하나의 골격 논리.
...
캐릭터 항목의 **리토폴로지 → 베이스 메시 템플릿**에서 템플릿을 고른다. 내장 템플릿은 두 종류다:

- **Blender Studio Human Base Meshes(CC0)** 전신 4종 — 남성/여성 × 스타일라이즈드(12.5k 쿼드)/사실적(10.6k 쿼드). 눈·입 동심 루프, 관절 루프, 얼굴 고밀도·팔다리 저밀도 분포를 갖춘 실제 애니메이션용 토폴로지다 (`lowpoly/templates/*.obj`, 출처는 `lowpoly/templates/NOTICE.md`). 인간형·크리처형의 기본값(자동)이다.
- **절차 생성 원형** 인간형/네발형(264 쿼드) — 손·발·입이 없는 편집용 뼈대. 네발형은 CC0 소스가 없어 이것만 있다.

템플릿 적합은 키로 균일 스케일 → 높이 슬라이스별 정면·측면 폭 맞춤 → 단계적 슈링크랩(타깃 노멀 투영 → 최근접 표면)으로 진행한다. 실측(늑대 몸체 셰이프, 남성 스타일라이즈드 템플릿): 몸체 표면의 68%가 템플릿 3cm 이내, 머리·몸통·다리는 따라가지만 팔이 얇게 늘어나고 무릎·꼬리 근처에 스파이크가 남는다 — **자동 적합은 출발점이고 Edit Mode 보정이 필요하다.** 그래서 기본 리토폴로지는 여전히 QuadriFlow다.
자동 선택은 캐릭터 유형이 **동물형**일 때 네발형이고 나머지는 인간형이다. 네발동물은 유형을 명시한다.

- **템플릿 불러오기**: 원본의 독립 복사본을 `LP3D_Template_Edit` 컬렉션에 넣는다.
  Edit Mode에서 체형·얼굴·관절을 수정한 뒤 Object Mode로 돌아와 **선택 메시 등록**을 누르면
  새 사용자 템플릿으로 저장된다. 내장 원본과 기존 등록본은 덮어쓰지 않는다.
- **.blend 등록**: 메시 오브젝트가 정확히 하나인 `.blend` 파일을 등록한다.
  FBX는 Blender로 가져온 뒤 선택 메시 등록을 사용한다. 기준은 정면 `-Y`, 위쪽 `+Z`, 중립 자세다.
  사용자 템플릿은 Blender config의 `lowpoly3d/templates/`에 저장되어 애드온 업데이트와 분리된다.
- 자체 제작 템플릿은 얼굴에 국소 루프, 팔·다리 관절에 지지 루프, 부위별 vertex group을 갖춘
  편집용 원형이다. 손가락·구강·눈꺼풀의 해부학적 구조나 리그는 포함하지 않는다.
  표면 투영만으로 자동 리깅 품질을 보장하지 않으며, 주둥이·꼬리·극단적인 비율은 템플릿 편집이 필요하다.
...
리토폴로지 결과 옆에는 **원본 하이폴리**가 `<이름>_원본` 자식 컬렉션에 숨겨져 함께 남는다 — 같은 원본으로 리토폴로지 설정을 바꿔 다시 돌릴 수 있고, 결과와 같은 자리·축척으로 겹쳐 있다. 결과가 나오는 즉시(모델링 완료·최종 완료) 원화·턴어라운드·셰이프 GLB·텍스처와 `.blend` 사본이 `~/Downloads/blender/<모드>/<이름>/` 폴더 하나로 저장된다 — 편집 중인 파일 경로는 바뀌지 않는다.

**단계 진행**(잡 옵션): 기본 "리토폴로지에서 정지"는 리토폴로지까지 하고 멈춘다. "셰이프에서 정지"를 고르면 **원본 셰이프만 만들고 멈춘다** — 원본을 보고 Edit Mode 에서 엣지를 **Mark Sharp** 로 그어 두면 그 선을 따라 와이어가 흐른다(복셀 리메시가 엣지를 지우므로 표시한 선을 좌표로 옮겨 리메시 메시에 다시 심고 QuadriFlow 의 `use_preserve_sharp` 로 넘긴다). 그 뒤 **[리토폴로지 시작]** → **[매핑 시작]** 순서다. "자동"은 끝까지 멈추지 않는다. 환경설정의 `좌우 대칭 와이어`(기본 켜짐)는 QuadriFlow 를 X축 대칭으로 돌려 좌우 흐름을 맞춘다.

**매핑 시점**(잡 옵션, 개별 매핑일 때): 리토폴로지가 끝나면 상태 **모델링 완료**로 멈춘다. 메시를 원하는 만큼 수정한 뒤 잡 패널의 **[매핑 시작]**을 누르면 그 시점의 메시를 그대로 언랩·6면도 채색·베이크한다. 매핑에 필요한 정보(요청문·턴어라운드·스타일·유형)는 컬렉션 커스텀 프로퍼티에 남아 Blender를 다시 열어도 유효하며, 매핑이 실패하면 메시는 그대로 두고 다시 누를 수 있다. "자동"은 기존처럼 리토폴로지 직후 매핑까지 이어간다.
...
환경설정 → **캐릭터**: 셰이프 생성 켜기/끄기, 서버 주소(기본 `http://127.0.0.1:8081`), 서버 토큰, 리토폴로지 목표 면수(기본 12,000 쿼드 ≈ 24,000 tris), 리토폴로지 방식, 캐릭터 기본 키. 통합 생성에서 서버가 응답하지 않으면 패널 로그에 남기고 코드 모델링으로 폴백한다. 템플릿 방식은 원형의 면수를 최대 두 단계 subdivision하므로 목표 면수와 정확히 같지는 않다.
