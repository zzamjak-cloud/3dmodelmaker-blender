...
# 보장되므로, UI의 "자동 반복" 1회 = 3턴(생성 1 + 비평·개선 2)으로 정의하고
# 최소 턴 전의 DONE은 무시한다.

TURNS_PER_CYCLE = 3   # 사이클당 턴 수: 생성 1 + 비평·개선 2
MIN_TURNS = 3         # 이 턴 전에는 에이전트의 DONE 선언을 무시한다


def total_turns(cycles: int) -> int:
    """자동 반복(사이클) 수를 내부 턴 수로 환산한다."""
    return max(1, cycles) * TURNS_PER_CYCLE
